...
parser.add_argument('-data_distribution', choices=['uniform', 'skew', 'gaussian'], help='data set distribution', default='skew')
parser.add_argument('-data_set_size', choices=[10000, 100000, 1000000], help='data set size', default=100000)
parser.add_argument('-training_set_size', choices=[10000, 100000, 1000000], help='training set size', default=100000)
parser.add_argument('-reward_comp_freq', choices=[1, 5, 10, 50, 100, 250, 1000], help='insertion reward computation frequency', default=10)
...
class Agent():
    ...
    def __init__(self, gamma, epsilon, alpha, maxMemorySize, batch_size, epsEnd=0.1, 
                 replace=20):
        ...
        self.Q_eval = one_layer_nn(alpha, 350, 450, 50)
        self.Q_next = one_layer_nn(alpha, 350, 450, 50)
    ...
    def learn(self):        
        if len(self.memory) > 0:
            ...
            loss.backward()
            print('layer1 ',self.Q_eval.layer1.weight)
            print('layer2 ', self.Q_eval.layer2.weight)
...
#init Agent
brain = Agent(gamma=0.95, epsilon=1.0, alpha=0.003, maxMemorySize=5000, batch_size=64, replace=20)
...
if __name__ == '__main__':
        ...
        # generate data set
        dataset_size = args.data_set_size
        side = 10.0**(-8)
        model_dataset = []
        count = 0
        if args.data_distribution == 'uniform':
                while count < dataset_size:
                    ...
                    if min(x,y) - side/2 > 0 and max(x,y) + side/2 < 100000:
                        model_dataset.append([x-side/2,x+side/2,y-side/2,y+side/2]) # the sequence of the 4 end points
                        count += 1
        ...
        for epo in range(int(args.epoch)):
                ...
                for i in range(args.training_set_size):
                        ...
                        states = tree.RetrieveSpecialInsertStates7() # states for insertion
                        ...
                        while states is not None: # we know if ptr points to leaf node when computing states
                                ...
                                states = tree.RetrieveSpecialInsertStates7()

                        tree.InsertWithLoc(0)
                        tree.DefaultSplit() # autmatically trigger default after insert_obj has reached a leaf node

                        brain.state_action_pairs.append([[0]*350, -1]) # terminal state for leaf node
                        ...
                        if (i+1) > args.max_entry and (i+1) % rf == 0:
                                ...
                                for k in range(rf):
                                        ...
                                        x_center = (model_dataset[i-k][1] + model_dataset[i-k][0])/2
                                        y_center = (model_dataset[i-k][3] + model_dataset[i-k][2])/2
        ...
        tree.Clear()
        reference_tree.Clear()
        for i in range(args.data_set_size):
        ...
        while k < 1000:
            ...
            if x - side > 0 and y - side > 0 and x + side < 100000 and y + side < 100000:
                ...
                # access_ratio = ((access_ratio * k) + reference_tree_access/tree_access) / (k+1)
                k = k + 1
        ...
        testing_query_area =  0.01/100 * (100000**2)
        side = (testing_query_area**0.5)/2
        k = 0
        while k < 1000:
            x = random.uniform(0, 1)*100000
        ...
        testing_query_area =  0.005/100 * (100000**2)
        side = (testing_query_area**0.5)/2
        k = 0
        while k < 1000:
            x = random.uniform(0, 1)*100000
